...
let state = {
    ...
    appointments: [], professionals: [], clients: [], services: [], transactions: [],
...
let expensesChart = null; // Variável global para o gráfico de despesas

const whatsappMessages = [
    "Olá {cliente}! Tudo bem? Estou passando para confirmar seu agendimento conosco. Podemos contar com sua presença?",
...
async function exportAnamnesisToPDF(client, record) {
    ...
    try {
        // Verifica se existe uma URL de logo no estado do salão
        if (state.salonInfo && state.salonInfo.logoUrl) {
            // Função auxiliar para converter a URL da imagem em formato de dados (Base64)
        ...
        doc.text("Logo não carregado", 15, 20); // Texto alternativo em caso de erro
    }
...
// --- INÍCIO DA SEÇÃO FINANCEIRA INTEGRADA ---
...
function getTransactionsForCurrentMonth() {
    ...
    // Receitas automáticas de agendamentos faturados
    const revenueFromAppointments = appointments
        ...
        .map(app => {
            ...
            return {
                id: `app-${app.id}`,
                date: app.date,
                description: `Serviço: ${service?.name || 'N/A'} (${client?.name || 'Cliente'})`,
    ...
    // Transações manuais (receitas e despesas)
...
function renderTransactionsList() {
    ...
    if (allTransactions.length === 0) {
        transactionsListEl.innerHTML = '<p class="text-center text-gray-500 p-4">Nenhum lançamento para este mês.</p>';
        return;
    }
    ...
    allTransactions.forEach(t => {
        ...
        el.innerHTML = `
            <div>
                ...
                <p class="text-xs text-gray-500">${t.date.toLocaleDateString('pt-BR')}</p>
                ${t.category ? `<p class="text-xs text-gray-500">Categoria: ${t.category}</p>` : ''}
            </div>
            <p class="font-bold text-lg ${isRevenue ? 'text-green-600' : 'text-red-600'}">
...
function renderFinancialOverview() {
    ...
    document.getElementById('overviewRevenue').textContent = `R$ ${totalRevenue.toFixed(2).replace('.', ',')}`;
    document.getElementById('overviewExpenses').textContent = `R$ ${totalExpenses.toFixed(2).replace('.', ',')}`;
    document.getElementById('overviewProfit').textContent = `R$ ${profit.toFixed(2).replace('.', ',')}`;
...
function renderProfessionalFinancials() {
        ...
        state.professionals.forEach(prof => {
            ...
            el.innerHTML = `
                ...
                <div class="mt-2 text-sm space-y-1">
                    ...
                    <div class="flex justify-between border-t pt-1 mt-1"><span>Líquido Salão:</span> <span class="font-bold text-green-600">R$ ${(profRevenue - commissionValue).toFixed(2).replace('.', ',')}</span></div>
                </div>`;
            detalhesFinanceiroEl.appendChild(el);
        });
        faturamentoTotalMesEl.textContent = `R$ ${totalMonthRevenue.toFixed(2).replace('.', ',')}`; // Elemento não existe mais, mas mantido para não quebrar
    } else if (state.role === 'professional' && state.professionalProfile) {
        const profAppointments = appointmentsInMonth.filter(app => app.professionalId === state.professionalProfile.id);
        const profRevenue = profAppointments.reduce((total, app) => {
            const service = state.services.find(s => s.id === app.serviceId);
            return total + (service?.price || 0);
        }, 0);
...
function renderFinanceiro() {
    if(state.role !== 'salonOwner') {
        renderProfessionalFinancials();
        return;
    };

    const financeiroCurrentMonthYear = document.getElementById('financeiroCurrentMonthYear');
    financeiroCurrentMonthYear.textContent = state.currentDate.toLocaleDateString('pt-BR', { month: 'long', year: 'numeric' });
    
    const activeTabEl = document.querySelector('.finance-tab.border-blue-500');
...
async function handleTransactionSubmit(e) {
    ...
    const data = {
        ...
        date: Timestamp.fromDate(new Date(dateValue + 'T12:00:00')), // Evita problemas de fuso
        type: type,
...
// --- FIM DA SEÇÃO FINANCEIRA INTEGRADA ---
...
function updateUIVisibility() {
    ...
    document.querySelector('button[data-page="equipePage"]').style.display = isOwner ? 'block' : 'none';
    document.querySelector('button[data-page="servicosPage"]').style.display = isOwner ? 'block' : 'none';
    document.querySelector('button[data-page="financeiroPage"]').style.display = 'block';
    document.getElementById('openClientModalBtn').style.display = 'block'; // Todos podem ver
    document.getElementById('totalClientesCard').style.display = isOwner ? 'block' : 'none';
    document.getElementById('totalServicosCard').style.display = isOwner ? 'block' : 'none';
    
    // Visibilidade dos paineis financeiros
    ...
    if(adminFinancialDetails) adminFinancialDetails.style.display = isOwner ? 'block' : 'none';
    if(professionalFinancialSummary) professionalFinancialSummary.style.display = isProfessional ? 'block' : 'none';
    if(financeTabs) financeTabs.style.display = isOwner ? 'flex' : 'none';

    // Se for profissional, esconde as abas e mostra direto o painel de profissional
    if (isProfessional) {
        document.querySelectorAll('.finance-tab-pane').forEach(p => p.classList.add('hidden'));
        document.getElementById('professionalTab').classList.remove('hidden');
    }
}
...
function main() {
    ...
    // Seletores do Módulo Financeiro
    const financeTabs = document.getElementById('financeTabs');
    ...
    addProfessionalForm.addEventListener('submit', async (e) => {
        ...
        try {
            if (id) {
                const { salonId, email, ...updateData } = data; // Não permite alterar email ou salonId ao editar
    ...
    addAppointmentForm.addEventListener('submit', async (e) => {
        ...
        if (!data.clientId || !data.professionalId || !data.serviceId) { alert('Preencha todos os campos.'); return; }
        try {
            if (id) { await updateDoc(doc(db, 'appointments', id), data); }
            else { await addDoc(collection(db, 'appointments'), data); }
            addAppointmentForm.reset(); addAppointmentModal.classList.add('hidden');
        } catch (err) { console.error(err); }
    });
    ...
    function openBlockTimeModal(blockId = null) {
        ...
        if (blockId) {
            ...
            if (state.role === 'salonOwner') profSelect.value = block.professionalId;
    ...
    blockDayForm.addEventListener('submit', (e) => {
        ...
        const dayHasAppointments = state.appointments.some(app =>
    ...
    anamnesisForm.addEventListener('submit', async (e) => {
        ...
        try {
            ...
            // Usar arrayUnion para adicionar o novo registro ao histórico
    ...
    financeTabs.addEventListener('click', (e) => {
        ...
        financeTabs.querySelectorAll('a').forEach(tab => {
    ...
    openExpenseModalBtn.addEventListener('click', () => openTransactionModal('expense'));
    openRevenueModalBtn.addEventListener('click', () => openTransactionModal('revenue'));
    ...
    onAuthStateChanged(auth, async (user) => {
        ...
        if (user) {
            ...
            const collectionsToListen = {
                ...
                transactions: "transactions" // Adicionado listener para transações
            };
            ...
            Object.keys(collectionsToListen).forEach(key => {
                ...
                const unsub = onSnapshot(q, (snapshot) => {
                    state[key] = snapshot.docs.map(doc => ({
                        id: doc.id,
                        ...doc.data(),
                        // Garante que o campo de data seja convertido para objeto Date
            ...
            state = { ...state, user: null, role: 'client', userSalonId: null, professionalProfile: null, salonInfo: null, appointments: [], professionals: [], clients: [], services: [], transactions: [] };
